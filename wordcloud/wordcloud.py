...
FONT_PATH = os.environ.get("FONT_PATH", os.path.join(os.path.dirname(__file__),
                                                     "DroidSansMono.ttf"))
STOPWORDS = list([x.strip() for x in open(os.path.join(os.path.dirname(__file__),
...
class WordCloud(object):
    ...
    def process_text(self, text):
        ...
        for l in line[1:-1]:
            ...
            #if time doesn't match with element, go to the next line
            if time != t:
                continue
            #if title includes stopwords, go to the next line 
            if self.isStopword(word) == 1:
                continue
